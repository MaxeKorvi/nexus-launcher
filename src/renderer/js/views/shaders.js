    ...
    body.innerHTML = `
      <div style="font-size:13px;line-height:1.6;color:var(--text-1);">
        ...
        <div style="background:rgba(255,180,0,0.1);border-left:3px solid #ffb400;padding:10px 12px;border-radius:4px;margin:12px 0;color:var(--text-1);font-size:12px;">
          Файлы на сторонних ресурсах загружаются авторами шейдеров. Nexus Launcher рекомендует убедиться в безопасности контента перед запуском.
        </div>
  ...
  window.Views.shaders = {
    ...
    showPreview(shader) {
      ...
      const header = `
        <div style="display:flex;gap:14px;align-items:flex-start;">
          <div style="width:64px;height:64px;border-radius:10px;background:var(--bg-2);background-size:cover;background-position:center;flex-shrink:0;${shader.icon ? `background-image:url('${shader.icon}')` : ''}"></div>
          <div style="flex:1;min-width:0;">
            <h3 style="margin:0 0 4px;font-size:17px;color:var(--text-0);">${esc(shader.title || '')}</h3>
            <div style="font-size:12px;color:var(--text-2);margin-bottom:6px;">
              Автор: <strong>${esc(shader.author || '—')}</strong> · Источник: <span class="badge" style="font-size:11px;padding:2px 6px;">${esc(sourceLabel(shader.source))}</span>
            </div>
            <div style="display:flex;flex-wrap:wrap;gap:8px;font-size:11px;color:var(--text-2);">
              ${shader.downloads ? `<span>⬇ ${Number(shader.downloads).toLocaleString('ru-RU')} скачиваний</span>` : ''}
            </div>
          </div>
        </div>
      `;
      ...
      const desc = `
        <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:8px;padding:12px;font-size:13px;line-height:1.6;color:var(--text-1);max-height:220px;overflow-y:auto;white-space:pre-wrap;">
          ${esc(shader.description || 'Описание отсутствует.')}
        </div>
      `;

      const versions = shader.mcVersions && shader.mcVersions.length ? `
        <div>
          ...
          <div style="display:flex;flex-wrap:wrap;gap:6px;max-height:80px;overflow-y:auto;">
            ${shader.mcVersions.map(v => `<span style="background:var(--bg-2);border:1px solid var(--border);padding:2px 8px;border-radius:4px;font-size:11px;color:var(--text-1);">${esc(v)}</span>`).join('')}
          </div>
        </div>
      ` : '';
      ...
      footer.innerHTML = `
        ${shader.url ? `<button class="btn outline" id="modal-preview-site">Открыть сайт в браузере</button>` : ''}
      ...
      const modal = Modal.open({
        title: esc(shader.title || 'Подробнее'),
        body,
        footer,
        size: 'large'
      });
      ...
      if (siteBtn && shader.url) {
        siteBtn.onclick = () => window.api.shell.openExternal(shader.url);
      }
      footer.querySelector('#modal-preview-close').onclick = () => modal.close();
      footer.querySelector('#modal-preview-install').onclick = () => {
        modal.close();
        this.installShader(shader);
      };
    },

    async installShader(item) {
      const target = this.currentTarget();
    ...
    async search() {
      ...
      try {
        ...
        list.querySelectorAll('[data-install]').forEach(b => b.onclick = () => this.installShader(r.hits[Number(b.dataset.install)]));
