    ...
    body.innerHTML = `
      <div style="font-size:13px;line-height:1.6;color:var(--text-1);">
        ...
        <div style="background:rgba(255,180,0,0.1);border-left:3px solid #ffb400;padding:10px 12px;border-radius:4px;margin:12px 0;color:var(--text-1);font-size:12px;">
          Файлы на сторонних ресурсах загружаются авторами ресурспаков. Nexus Launcher рекомендует убедиться в безопасности контента перед запуском.
        </div>
  ...
  window.Views.resourcepacks = {
    ...
    async render() {
      ...
      c.innerHTML = `
        <div class="view" style="padding:24px;">
          <h1>Менеджер ресурспаков</h1>
          <p style="color:var(--text-2);margin-bottom:16px;">Ресурспак ставится строго в <b>resourcepacks/ выбранной установленной версии</b>. Выберите версию, каталог и файл попадёт туда.</p>
          <div class="search-bar" style="display:grid;grid-template-columns:1.2fr 1.35fr 0.75fr 0.9fr auto;gap:10px;align-items:center;">
            <input class="input" id="rp-search" placeholder="Поиск ресурспака…" value="${esc(this.query)}">
            <select class="input" id="rp-target">
            ...
            <input class="input" id="rp-version" placeholder="Версия Minecraft" value="${esc(this.mcVersion)}" readonly>
            <select class="input" id="rp-prefer">
            ...
            <button class="btn ghost" id="rp-installed">Установленные</button>
          </div>
          <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin:12px 0 16px;">
            ${['curseforge','modrinth','minecraft-inside'].map(src => `<label class="pill-mini" style="display:flex;align-items:center;gap:6px;padding:8px 12px;cursor:pointer;"><input type="checkbox" data-rp-src="${src}" ${this.sources.includes(src) ? 'checked' : ''}><span>${sourceLabel(src)}</span></label>`).join('')}
          </div>
          ...
          <div id="rp-context" style="margin:10px 0 18px;color:var(--text-2);font-size:12px;"></div>
          <div id="rp-list" class="card grid" style="grid-template-columns:1fr;"></div>
          <div style="display:flex;justify-content:center;margin-top:12px;gap:8px;">
            <button class="btn ghost" id="rp-prev" style="${this.page > 0 ? '' : 'display:none;'}">← Назад</button>
            <span id="rp-page" style="padding:8px 14px;">Стр. ${this.page + 1}</span>
            <button class="btn ghost" id="rp-next">Вперёд →</button>
          </div>
        </div>`;

      let timer = null;
      document.getElementById('rp-search').oninput = (e) => {
        this.query = e.target.value;
        clearTimeout(timer);
        timer = setTimeout(() => { this.page = 0; this.search(); }, 350);
      };
      document.getElementById('rp-search').onkeydown = (e) => { if (e.key === 'Enter') { clearTimeout(timer); this.query = e.target.value; this.page = 0; this.search(); } };
      document.getElementById('rp-target').onchange = (e) => {
        const target = this.targets().find(v => targetKey(v) === e.target.value);
        if (target) this.applyTarget(target, true);
        this.page = 0;
        this.render();
      };
      document.getElementById('rp-prefer').onchange = (e) => { this.preferSource = e.target.value; this.page = 0; this.search(); };
      document.querySelectorAll('[data-rp-src]').forEach(x => x.onchange = () => {
        this.sources = Array.from(document.querySelectorAll('[data-rp-src]:checked')).map(y => y.dataset.rpSrc || y.getAttribute('data-rp-src'));
        if (!this.sources.length) this.sources = ['modrinth'];
        this.page = 0;
        this.search();
      });
      document.getElementById('rp-installed').onclick = () => this.showInstalled();
      document.getElementById('rp-prev').onclick = () => { if (this.page > 0) { this.page--; this.search(); } };
      document.getElementById('rp-next').onclick = () => { this.page++; this.search(); };
      this.updateContext();
      this.search();
    },

    updateContext() {
      const el = document.getElementById('rp-context');
      if (!el) return;
      ...
      const rootDir = target.rootDir || target.path;
      el.textContent = `Каталог ограничен выбранной версией Minecraft: ${this.mcVersion || '—'}. Установка пойдёт сюда: ${rootDir}/resourcepacks`;
    },
    ...
    showPreview(pack) {
      ...
      const header = `
        <div style="display:flex;gap:14px;align-items:flex-start;">
          <div style="width:64px;height:64px;border-radius:10px;background:var(--bg-2);background-size:cover;background-position:center;flex-shrink:0;${pack.icon ? `background-image:url('${pack.icon}')` : ''}"></div>
          <div style="flex:1;min-width:0;">
            <h3 style="margin:0 0 4px;font-size:17px;color:var(--text-0);">${esc(pack.title || '')}</h3>
            <div style="font-size:12px;color:var(--text-2);margin-bottom:6px;">
              Автор: <strong>${esc(pack.author || '—')}</strong> · Источник: <span class="badge" style="font-size:11px;padding:2px 6px;">${esc(sourceLabel(pack.source))}</span>
            </div>
            <div style="display:flex;flex-wrap:wrap;gap:8px;font-size:11px;color:var(--text-2);">
              ${pack.downloads ? `<span>⬇ ${Number(pack.downloads).toLocaleString('ru-RU')} скачиваний</span>` : ''}
            </div>
          </div>
        </div>
      `;
      ...
      const desc = `
        <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:8px;padding:12px;font-size:13px;line-height:1.6;color:var(--text-1);max-height:220px;overflow-y:auto;white-space:pre-wrap;">
          ${esc(pack.description || 'Описание отсутствует.')}
        </div>
      `;

      const versions = pack.mcVersions && pack.mcVersions.length ? `
        <div>
          ...
          <div style="display:flex;flex-wrap:wrap;gap:6px;max-height:80px;overflow-y:auto;">
            ${pack.mcVersions.map(v => `<span style="background:var(--bg-2);border:1px solid var(--border);padding:2px 8px;border-radius:4px;font-size:11px;color:var(--text-1);">${esc(v)}</span>`).join('')}
          </div>
        </div>
      ` : '';
      ...
      footer.innerHTML = `
        ${pack.url ? `<button class="btn outline" id="modal-preview-site">Открыть сайт в браузере</button>` : ''}
      ...
      const modal = Modal.open({
        title: esc(pack.title || 'Подробнее'),
        body,
        footer,
        size: 'large'
      });
      ...
      if (siteBtn && pack.url) {
        siteBtn.onclick = () => window.api.shell.openExternal(pack.url);
      }
    ...
    async installPack(item) {
      ...
      checkMinecraftInsideDisclaimer(item, async () => {
        const rootDir = target.rootDir || target.path;
        try {
          await window.api.invoke('resourcepacks:install', {
            ...item,
            rootDir,
            gameDir: rootDir,
          ...
          Toast.success('Ресурспак установлен', `${rootDir}/resourcepacks`);
          this.search();
    ...
    async search() {
      ...
      try {
        const r = await window.api.invoke('resourcepacks:list', {
          query: this.query,
        ...
        const pageEl = document.getElementById('rp-page');
        ...
        const installed = rootDir ? await window.api.invoke('resourcepacks:list-installed', rootDir) : [];
        ...
        list.querySelectorAll('[data-install]').forEach(b => b.onclick = () => this.installPack(r.hits[Number(b.dataset.install)]));
    ...
    async showInstalled() {
      ...
      const rootDir = target.rootDir || target.path;
      const list = await window.api.invoke('resourcepacks:list-installed', rootDir);
      if (!list.length) { el.innerHTML = `<div class="empty-state">Нет установленных файлов в ${esc(rootDir)}/resourcepacks</div>`; return; }
